...
public class VolumeSlider : Selectable, IDragHandler, IInitializePotentialDragHandler, ICanvasElement
{
    [Serializable]
    /// <summary>
    ...
    [SerializeField] private float inclineAngle = 45f;
    ...
    ///</code>
    /// </example>
    public Slider.Direction direction { get { return m_Direction; } set { if (SetStruct(ref m_Direction, value)) UpdateVisuals(); } }
    ...
    ///</code>
    /// </example>
    public float minValue { get { return m_MinValue; } set { if (SetStruct(ref m_MinValue, value)) { Set(m_Value); UpdateVisuals(); } } }
    ...
    ///</code>
    /// </example>
    public float maxValue { get { return m_MaxValue; } set { if (SetStruct(ref m_MaxValue, value)) { Set(m_Value); UpdateVisuals(); } } }
...
#pragma warning restore 649
    ...
    Axis axis2 { get { return (m_Direction == Slider.Direction.LeftToRight || m_Direction == Slider.Direction.RightToLeft) ? Axis.Vertical : Axis.Horizontal; } }
    ...
    private void UpdateVisuals()
    {
        ...
        float inclineRatio = Mathf.Tan(inclineAngle * Mathf.Deg2Rad); // convert angle to a ratio
        ...
        if (m_FillContainerRect != null)
        {
            ...
            else
            {
                if (reverseValue)
                {
                    ...
                    anchorMin[(int)axis2] = 1 - normalizedValue * inclineRatio;
                }
                else
                {
                    anchorMax[(int)axis] = normalizedValue;
                    anchorMax[(int)axis2] = normalizedValue * inclineRatio;
                }
            }
        ...
        if (m_HandleContainerRect != null) // moving handle
        {
            m_Tracker.Add(this, m_HandleRect, DrivenTransformProperties.Anchors);
            Vector2 anchorMin = Vector2.zero;
            Vector2 anchorMax = Vector2.one;
            anchorMin[(int)axis] = anchorMax[(int)axis] = (reverseValue ? (1 - normalizedValue) : normalizedValue);
            anchorMin[(int)axis2] = anchorMax[(int)axis2] = (reverseValue ? (1 - normalizedValue * inclineRatio) : normalizedValue *inclineRatio);
    ...
    public bool GetRelativeMousePositionForDrag(PointerEventData eventData, ref Vector2 position)
    {
#if UNITY_EDITOR
